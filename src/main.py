...
class PersonaDrivenProcessor:
    ...
    def __init__(self, persona: str, job: str):
        ...
        self.relevance_scorer = RelevanceScorer(max_features=5000)
        self.output_formatter = OutputFormatter()
    
    def process_single_pdf(self, pdf_path: str, output_dir: str):
        """Process a single PDF file and return analysis data"""
        logger.info(f"📄 Processing: {Path(pdf_path).name}")
        start_time = time.time()
        
        try:
            # Extract text from PDF
            document_data = self.pdf_processor.extract_text_with_structure(Path(pdf_path))
            document_data['filename'] = Path(pdf_path).name
            ...
            document_text = self._extract_document_text(processed_doc)
            document_id = Path(pdf_path).stem
            persona_query = f"{self.persona}: {self.job}"
            
            # Score relevance
            scored_sections = self.relevance_scorer.score_documents(
                [document_text], [document_id], persona_query
            )
    ...
    def generate_consolidated_output(self, all_results, input_documents, config, output_dir):
        """Generate consolidated output in the required format"""
        try:
            # Extract successful results
            successful_results = [result for result in all_results if result[1] is not None]
            ...
            logger.error(f"Error generating consolidated output: {e}")
            return False
    
    def _extract_document_text(self, processed_doc: dict) -> str:
        """Extract all text content from a processed document"""
        text_parts = []
        
        for section in processed_doc.get('processed_sections', []):
            # Add title if available
            title = section.get('cleaned_title', '').strip()
            if title:
                text_parts.append(title)
            
            # Add main content
            content = section.get('cleaned_content', '').strip()
            if content:
